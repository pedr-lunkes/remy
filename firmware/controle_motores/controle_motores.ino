...
#define dir2 32 //inverte a direcao
...
const float MAX_LINEAR = 1.0;
const float MAX_ANGULAR = 1.0;
...
void setup() {
  ...
  pinMode(ha1_pin, INPUT_PULLUP);
  pinMode(hb1_pin, INPUT_PULLUP); //os halls sao entrada e os pwm sao saída
  pinMode(hc1_pin, INPUT_PULLUP);
  pinMode(pwm1, OUTPUT);
  pinMode(dir1, OUTPUT);

  pinMode(ha2_pin, INPUT_PULLUP);
  pinMode(hb2_pin, INPUT_PULLUP);
  pinMode(hc2_pin, INPUT_PULLUP);
  pinMode(pwm2, OUTPUT);
  pinMode(dir2, OUTPUT);
...
void loop() {
  if (Serial.available()) {
    ...
    float linear = input.substring(0, spaceIndex).toFloat();
    float angular = input.substring(spaceIndex + 1).toFloat();
    ...
    digitalWrite(dir1, leftSpeed >= 0 ? HIGH : LOW);
    digitalWrite(dir2, rightSpeed >= 0 ? HIGH : LOW);

    // Map from -1..1 to 0..255 PWM
    int pwmLeft = (int)(fabs(leftSpeed) * MAX_PWM);
    int pwmRight = (int)(fabs(rightSpeed) * MAX_PWM);

    Serial.print("PWM Left: "); Serial.println(pwmLeft);
    Serial.print("PWM Right: "); Serial.println(pwmRight);

    analogWrite(pwm1, pwmLeft); //Controlar velocidade do motor 1      <--------------------- AQUI MUDAS AS VELOCIDADESSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS
